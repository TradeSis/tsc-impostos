...
if (isset($LOG_CAMINHO)) {
    ...
    $identificacao = date("dmYHis") . "-PID" . getmypid() . "-" . "fisprocesso_inserir";
    if (isset($LOG_NIVEL)) {
        if ($LOG_NIVEL >= 1) {
...
if (isset($jsonEntrada['nomeProcesso'])) {

    try {

        $progr = new chamaprogress();
        $retorno = $progr->executarprogress("impostos/app/1/fisprocesso_inserir",json_encode($jsonEntrada));
